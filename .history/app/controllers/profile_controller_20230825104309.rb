...
class ProfileController < ApplicationController
  ...
  def show
    if account_signed_in? && current_account.id == @account.id
      ...
      @show_picture=true
    elsif  @account.private==true
      if private_accounts_request_accepted.include(@account.id)
        byebug
        current_account_posts = Post.where(account_id: @account.id)
        @linked_posts=current_account_posts
        @show_picture=true
      else
        @show_picture=false
  ...
  TypeError in ProfileController#show
wrong argument type Integer (expected Module)
...
18
      ...
      if private_accounts_request_accepted.exists?(@account.id)
        byebug
        current_account_posts = Post.where(account_id: @account.id)
        @linked_posts=current_account_posts
