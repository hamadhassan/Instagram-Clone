...
class ProfileController < ApplicationController
  ...
  def show
    if account_signed_in? && current_account.id == @account.id
      ...
      @show_picture=true
    elsif @account.private == false


    elsif  @account.private==true
      if private_accounts_request_accepted.include(@account.idid)
      current_account_posts = Post.where(account_id: @account.id, private: true)
      @linked_posts=current_account_posts
      @show_picture=true
      else
        @show_picture=false
      ...
      current_account_posts = Post.where(account_id: @account.id, private: false)
      @linked_posts=current_account_posts
      @show_picture=true
    end
  end
