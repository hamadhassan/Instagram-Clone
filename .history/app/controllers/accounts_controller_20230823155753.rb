...
class AccountsController < ApplicationController
  ...
  def create_follower(following_id, follower_id, accepted)
    ...
    if following_account && follower_account
      # Check if the follower already exists to avoid duplication
      existing_follower = Follower.find_by(following_id: following_id, follower_id: follower_id)

      unless existing_follower
        Follower.create!(following_id: following_id, follower_id: follower_id, accepted: accepted)
      end
    else
      # Handle the case where either the following_account or follower_account doesn't exist
      # You can raise an error, return a specific response, or log a message here.
    end
  end
