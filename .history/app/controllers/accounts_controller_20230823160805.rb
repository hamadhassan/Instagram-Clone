...
class AccountsController < ApplicationController
  ...
  def index
    ...
    @posts = Post.joins(account: :followers).where(followers: { accepted: true ,following_id: current_account.id})+ Post.where(account_id: following_ids, private: false)
  ...
  def create_follower(following_id,follower_id,accepted)
    Follower.create!( follower_id:, accepted:)
  end
