...
class AccountsController < ApplicationController
  ...
  def index
    @comment = Comment.new
    following_ids = Follower.where(following_id: current_account.id).map(&:following_id)
    ...
    @posts = Post.joins(account: :followers).where(followers: { accepted: true ,following_id: current_account.id}).distinct
  end
