...
class AccountsController < ApplicationController
  ...
  def profile
    if current_account == @account
      ...
      @linked_posts = Post.where(account_id: @account, private: false)
      @show_picture = true
    elsif current_account.private?
      @linked_posts=     @posts = Post.joins(account: :followers).where(followers: { accepted: true ,following_id: current_account.id})
