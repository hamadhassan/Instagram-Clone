...
class AccountsController < ApplicationController
  ...
  def profile
    ...
    if current_account == @account
      ...
      following_ids = Follower.where(following_id: current_account.id).map(&:follower_id)
      @linked_posts = Post.where(account_id: following_ids, private: true)
      @show_picture = true
    elsif current_account != @account && !current_account.private?
      @linked_posts= Post.joins(account: :followers).where(followers: { accepted: false ,following_id: current_account.id})
    else
      @show_picture = false
    end
  end
