...
class AccountsController < ApplicationController
  ...
  def index
    ...
    accounts_with_accepted_following = Account.joins(:following_relationships)
    .where(relationships: { accepted: true })
    private_posts = Post.where(private: true, account: accounts_with_accepted_following)

    private_posts = Post.where(account_id: current_account.id, private: true)
