...
class AccountsController < ApplicationController
  ...
  def index
    ...
    accepted_following_accounts = Account.joins(:following_accounts).where(relationships: { accepted: true }).distinct
    private_posts = Post.where(account_id: following_ids,private: true, account: accepted_following_accounts)


    following_ids_ = current_account.following.pluck(:id)
