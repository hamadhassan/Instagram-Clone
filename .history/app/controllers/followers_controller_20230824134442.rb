...
class FollowersController < ApplicationController
  def index
    ...
    @private_accounts = Account.joins(:relationships)
                 .where(relationships: { follower_id: current_account_id, accepted: false }).distinct

    @private_accounts = Account.joins(:followers)
                               .where(followers: { following_id: current_account_id, accepted: false })
                               .distinct
