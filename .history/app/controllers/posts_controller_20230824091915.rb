...
class PostsController < ApplicationController
  ...
  def update
    ...
    # Store the current image attachments in a variable
    previous_images = @post.images_attachments

    if @post.update(post_params)
      if params[:post][images: []].present?
        # User has uploaded new images, attach them
        @post.images.attach(params[:post][:images])
      else
        # User has not uploaded new images, reattach the previous ones
        @post.images.attach(previous_images.map(&:blob)) if previous_images.present?
      end
