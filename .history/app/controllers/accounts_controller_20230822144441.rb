...
class AccountsController < ApplicationController
  ...
  def follower_accepted?
    # Replace this with code that checks if the follower status is accepted
    # For example, if you have a User model with followers, you might do something like this:
    if @account.followers.where(status: 'accepted').exists?
      return true
    else
      return false
    end
