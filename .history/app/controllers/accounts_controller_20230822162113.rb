...
class AccountsController < ApplicationController
  ...
  before_action :set_account, only: [:profile,:show]
  def index
    @comment = Comment.new
  ...
  def follow_account
    ...
    existing_follower = Follower.find_by(follower_id: current_account.id, following_id: following_id)
    if following_account.private?
      Follower.create!(follower_id: current_account.id, following_id: following_id, accepted: false) unless existing_follower
    else
      Follower.create!(follower_id: current_account.id, following_id: following_id, accepted: true) unless existing_follower
    end
    redirect_to dashboard_path
  end

  def follow_request
    @follow_request=Follower.where(follower_id: current_account.id).map(&:following_id)
  end
