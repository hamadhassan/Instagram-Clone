...
class PostsController < ApplicationController
  ...
  def update
    ...
    # First, update description and privacy attributes
    if @post.update(description: params[:post][:description], private: params[:post][:private])
      # Then, handle image updates if images have changed
      if @images_changed
        # Assuming you have a method like 'update_images' to handle image updates
        if @post.update_images(params[:post][:images])
          redirect_to post_path(@post), notice: 'Post was successfully updated.'
        else
          flash[:alert] = 'Failed to update images.'
          render :edit
        end
      else
        redirect_to post_path(@post), notice: 'Post was successfully updated.'
      ...
      # Handle the case where the update failed, perhaps display an error message.
      render :edit
    end
