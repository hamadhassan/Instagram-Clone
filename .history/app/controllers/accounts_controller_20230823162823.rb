...
class AccountsController < ApplicationController
  ...
  def index
    ...
    @posts = Post.joins(account: :followers).where("followers.accepted = ? AND (account_id = ? OR followers.following_id = ?)", true, current_account.id, current_account.id).distinct
  end
