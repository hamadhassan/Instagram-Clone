...
class AccountsController < ApplicationController
  ...
  def profile
    same_user = @signed_in_user == @account

    if same_user || !@account.private?
      # If the signed-in user is the same as the user whose profile is being viewed
      # OR the profile is public, show both public and private posts
      @linked_posts = @account.posts
      @show_picture = true
    else
      # If the profile is private and the signed-in user is not the same,
