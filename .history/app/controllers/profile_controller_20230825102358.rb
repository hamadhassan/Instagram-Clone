...
class ProfileController < ApplicationController
  ...
  def show
    current_account_posts_public = Post.where(account_id: current_account.id) if current_account.id==@account.id
    # if account_signed_in? && current_account.id == @account.id
      current_account_posts_private = Post.where(account_id: current_account.id)
    # end
